...
try {
        ...
        const response=await axios.delete(`${import.meta.env.VITE_BASE_URL}/events/delete-event/${id}`);
    ...
    const getAllEvents = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/events/get-events`);
...
/>
            ...
            <div>
                ...
                <button type="button" className="bg-red-600 p-3 text-white rounded-2xl"  onClick={() => setshowCreateForm(true)}>Create  New Event</button>
                        ...
                        events.map((Event) => {
                            ...
                            return (
                                <div key={Event?.id} className=" bg-white   rounded-xl shadow-md overflow-hidden hover:shadow-xl transition ">
                                    <img src={Event.bannerUrl} className='w-full h-48 object-cover'></img>
                                    ...
                                    <div className='border-2 bg-black text-white' >
                                        ...
                                        <h4>{Event?.eventDescription}</h4>
                                          <button type='button' onClick={()=>handleDelete(Event.id) }className="border-2 bg-red-200">Delete</button>
                                              <button type='button' onClick={()=>{
                                                setSelectedEvent(Event),
                                                setShowUpdateForm(true)
                                              } }className="border-2 bg-red-200">update</button>
                                    </div>
